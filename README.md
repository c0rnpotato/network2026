...
* **Major-parameter**
  * **G (또는 ch_info.p)**: 시스템 전체의 총 트래픽 부하(Offered Load) `G`와 이를 기반으로 각 스테이션이 매 틱(tic)마다 패킷을 생성할 확률 `p`입니다. 스테이션 수와 슬롯당 틱 수에 반비례하여 노드별 생성 확률이 결정됩니다.
  * **success**: 충돌 없이 최종 전송에 성공한 패킷의 총 개수입니다. 전송 도중 새로운 패킷이 유입되어 충돌이 발생하면, 이전에 선제적으로 증가시켰던 성공 카운트를 다시 차감(`success--`)하는 방식으로 검증합니다.
  * **using**: 현재 채널을 점유하고 있는 패킷의 남은 전송 시간(Tic 수)을 나타내는 다운 카운터입니다. 패킷 전송이 시작되면 `tic_per_slot - 1`로 초기화되며, 매 틱마다 1씩 감소하여 0이 될 때까지 채널 점유 상태를 유지합니다.
  * **score**: 현재 전송 중인 패킷이 충돌 없이 정상적으로 진행되고 있는지를 나타내는 상태 플래그(부울 상태)입니다. 최초 전송 시 유일한 전송이면 `1`로 시작하지만, 패킷이 끝나기 전에 다른 전송 요청이 들어오면 충돌로 판정되어 `0`으로 바뀝니다.
  * **channel (또는 tmp/ChannelState)**: 매 틱마다 스테이션들의 전송 요청 수(`req`)를 취합하여 결정되는 채널의 즉각적인 상태입니다. 요청이 없으면 `FREE`, 1개이면 `FULL`, 2개 이상 발생하면 이산적 충돌을 뜻하는 `FAIL`로 매핑됩니다.
